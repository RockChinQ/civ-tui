...
func defaultOpts() GameOptions {
	return GameOptions{NumAICivs: 1, MapSize: MapSizeSmall, Difficulty: 1}
}
...
func TestCombat(t *testing.T) {
	...
	var player, enemy *Unit
	for _, u := range g.Units {
		if u.CivID == 1 && u.Type == UnitWarrior {
			player = u
		}
		if u.CivID == 2 && u.Type == UnitWarrior {
			enemy = u
		}
	}
...
func TestCityProductionYields(t *testing.T) {
	...
	var settler *Unit
	for _, u := range g.Units {
		if u.CivID == 1 && u.Type == UnitSettler {
			settler = u
			break
		}
	}
	...
	if tile == nil || !Terrains[tile.Terrain].Passable {
...
func TestTechResearch(t *testing.T) {
	...
	completed := civ.ProcessResearch(tech.Cost, AllTechs)
...
func TestMapGeneration(t *testing.T) {
	for _, size := range []MapSize{MapSizeSmall, MapSizeMedium, MapSizeLarge} {
		cfg := MapSizes[size]
		m := NewGameMap(42, size)
		...
		for y := 0; y < m.Height; y++ {
			for x := 0; x < m.Width; x++ {
				if Terrains[m.Tiles[y][x].Terrain].Passable {
					passable++
				}
			}
		}
