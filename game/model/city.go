...
func (c *City) FoodYield(tile *Tile) int {
	base := 2
	if tile != nil {
		base += Terrains[tile.Terrain].Food
		imp := tile.Improvement
		if imp != ImprovementNone {
			base += Improvements[imp].FoodBonus
		}
	}
	if c.Buildings[BuildingGranary] {
		base += BuildingDefs[BuildingGranary].FoodBonus
	}
	return base
}

func (c *City) ProductionYield(tile *Tile) int {
	base := 1
	if tile != nil {
		base += Terrains[tile.Terrain].Production
		imp := tile.Improvement
		if imp != ImprovementNone {
			base += Improvements[imp].ProdBonus
		}
	}
	if c.Buildings[BuildingBarracks] {
		base += BuildingDefs[BuildingBarracks].ProdBonus + 1
	}
	return base
}

func (c *City) GoldYield(tile *Tile) int {
	base := 1
	if tile != nil {
		base += Terrains[tile.Terrain].Gold
		imp := tile.Improvement
		if imp != ImprovementNone {
			base += Improvements[imp].GoldBonus
		}
	}
	if c.Buildings[BuildingMarket] {
		base += BuildingDefs[BuildingMarket].GoldBonus
	}
	return base
}
...
func (c *City) ProcessTurn(tile *Tile) (unitBuilt *ProductionItem, msg string) {
	c.Food += c.FoodYield(tile)
	...
	if len(c.ProductionQ) > 0 {
		...
		c.Production += c.ProductionYield(tile)
...
func AvailableBuildings(civTechs map[string]bool) []BuildingType {
	...
	for _, bt := range []BuildingType{BuildingGranary, BuildingBarracks, BuildingMarket, BuildingLibrary, BuildingWalls} {
